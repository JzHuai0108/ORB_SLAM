...
// TODO: check up the aliasing effect of Eigen Matrices, c.f. http://eigen.tuxfamily.org/dox-devel/group__TopicAliasing.html
// use noalias() or eval() when necessary, be mindful of in place resize, transpose.
...
/*void IMUProcessor::freeInertial(std::string output_file, double finish_time)
{
    ...
    assert(dt>0);
    IMUErrorModel<double> iem(speed_bias_1.tail<6>());
    iem.estimate(measurements[0].segment<3>(1), measurements[0].tail<3>());
    ...
    ScaViSLAM::strapdown_local_quat_bias( r_old, v_old, q_old, iem.a_est, iem.w_est,
                                          dt, imu_.gwomegaw, &r_new, &v_new, &q_new);
    if(bPredictCov)
    {
        ScaViSLAM::sys_local_dcm_bias(r_old, v_old, q_old, iem.a_est, iem.w_est,
    ...
    for (; jack<measurements.size()-1;++jack){
        ...
        iem.estimate(measurements[jack].segment<3>(1), measurements[jack].tail<3>());
