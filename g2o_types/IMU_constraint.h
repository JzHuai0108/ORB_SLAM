...
// It is advised to define the computeError function in a G2o Edge with an operator()
// so that the class may be used in a ceres solver problem. And to make covariance computation easy,
// normalize the cost function to unit 1 with sqrt(Cov(e))
// A good example of defining such cost functions is given in
...
#include "eigen_utils.h" //for rvec2quat, skew3d
#include "timegrabber.h" //imugrabber

#include "anchored_points.h" //G2oVertexSE3
...
{
public:   
    IMUProcessor(const std::string imu_file, const double sample_interval,const G2oIMUParameters &imu, IMUFileType fileType= PlainText);
